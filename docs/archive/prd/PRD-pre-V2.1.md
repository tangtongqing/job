...
> **v4 修订说明**（2026-07-28）：
> 1. 岗位库外投递支持最小字段补录，并原子创建岗位、投递与初始事件；
> 2. 招聘通知中的明确面试/测评时间，经用户确认后写入计划事件；
> 3. 状态事件与计划事件共享事务，详情时间线与近期安排读取同一数据源；
> 4. 当前实现与验收状态见 `docs/engineering/P0-CLOSED-LOOP.md`。
>
> **v3 修订说明**（2026-06-22）：
...
| 日期 | 2026年7月28日 |
| 状态 | 当前产品基线 |
| 修订记录 | v2：状态机/AC/数据模型/范围边界；v3：9 状态/纠错/事件表；v4：通知时间计划事件与库外投递补录 |

**关联文档**：
- `docs/research/competitor-analysis.md` — 竞品调研（TASK-001）
- `docs/product/MRD.md` — 市场需求文档（MRD）
- `docs/product/JTBD.md` — 痛点与 Job Stories
- `docs/product/PERSONAS.md` — 行为分群与假设画像
- `docs/product/PROJECT-CHARTER.md` — 立项与范围决策
