...
const DECISIONS = [
  {
    index: "01",
    title: "先重建主线，再扩展功能",
    body: "将原本并列的采集、看板、投递功能，重组成“发现 → 收藏/待投递 → 创建投递 → 更新状态 → 近期安排 → 复盘”的连续路径。",
  },
  {
    index: "02",
    title: "收藏和待投递必须分开",
  ...
  },
  {
    index: "04",
    title: "官网与作品集各自完成任务",
    body: "官网面向潜在用户，讲价值与真实产品；本页单独解释问题、取舍、验证与下一步，避免把 SaaS 首页做成项目汇报。",
  },
];
    ...
    <main className="bg-[#f7f7f5] text-[#171719] dark:bg-[#0d0d0f] dark:text-white">
        ...
        <div className="mx-auto max-w-6xl">
          <motion.div initial={reduceMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }} className="grid gap-10 lg:grid-cols-[1.35fr_.65fr] lg:items-end">
            ...
            <dl className="grid grid-cols-2 gap-x-6 gap-y-5 border-t border-black/10 pt-5 text-xs dark:border-white/10 lg:grid-cols-1">
              ...
              <CaseMeta label="当前状态" value="Public Beta · 本地完整演示" />
              <CaseMeta label="约束" value="不使用虚构用户、收入与转化数据" />
            </dl>
          </motion.div>
      ...
      <section className="px-5 py-20 md:py-28">
        <div className="mx-auto max-w-6xl">
          <div className="grid gap-12 lg:grid-cols-[.7fr_1.3fr]">
            <div><p className="text-xs font-medium text-[#6366f1]">04 · BUILD & EVIDENCE</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.035em] md:text-4xl">验证的是系统完整性，<br />不是虚构增长。</h2></div>
            <div>
              <div className="grid gap-3 sm:grid-cols-3"><Evidence value="99" label="后端自动化测试通过" /><Evidence value="14" label="前端路由完成生产构建" /><Evidence value="3" label="官网、Demo、案例三类界面" /></div>
              <div className="mt-8 rounded-2xl border border-black/[0.08] bg-white p-5 dark:border-white/10 dark:bg-[#111113]">
                ...
                <p className="mt-2 text-sm leading-7 text-black/55 dark:text-white/55">当前证据证明“产品流程可以完整演示、契约有测试、界面可以构建”，不证明留存、付费意愿或求职成功率。真实用户研究、行为数据与通知效果属于下一阶段。</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="border-t border-black/[0.07] bg-white px-5 py-20 dark:border-white/10 dark:bg-[#111113] md:py-28">
        <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1fr_1fr]">
          <div><p className="text-xs font-medium text-[#6366f1]">05 · NEXT</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.035em] md:text-5xl">从完整演示，走向长期使用。</h2><p className="mt-5 max-w-xl text-sm leading-7 text-black/55 dark:text-white/55">未来计划不会阻塞当前 Demo。只有真实使用暴露出持续价值后，才进入账号同步、通知、数据托管与商业化。</p></div>
          <ol className="space-y-3">
            {["邀请目标用户完成 5 次任务型走查", "验证收藏→待投递→投递的实际转化与流失点", "基于提醒命中率评估通知渠道", "确认多设备同步需求后再引入账户体系"].map((item, index) => <li key={item} className="flex min-h-16 items-center gap-4 rounded-xl border border-black/[0.08] bg-[#f7f7f5] px-4 dark:border-white/10 dark:bg-white/[0.04]"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#eef2ff] text-xs font-semibold text-[#4f46e5] dark:bg-indigo-950 dark:text-indigo-300">{index + 1}</span><span className="text-sm">{item}</span></li>)}
          </ol>
        </div>
      </section>
